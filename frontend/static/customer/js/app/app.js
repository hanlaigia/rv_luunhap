...
(function () {
  ...
  function syncSmartSearchPlaceholder() {
    var input = document.querySelector(".smart-search input[name='q'][data-mobile-placeholder]");
    if (!input) return;
    var desktop = input.getAttribute("placeholder") || "";
    var mobile = input.getAttribute("data-mobile-placeholder") || desktop;
    if (!input.dataset.desktopPlaceholder) {
      input.dataset.desktopPlaceholder = desktop;
    }
    function apply() {
      var isMobile = window.matchMedia("(max-width: 991.98px)").matches;
      input.placeholder = isMobile ? mobile : input.dataset.desktopPlaceholder;
    }
    apply();
    window.addEventListener("resize", apply);
  }

  function bindGuestDrawerToggles() {
    document.querySelectorAll(".rovva-guest-drawer__toggle").forEach(function (btn) {
  ...
  document.addEventListener("DOMContentLoaded", function () {
    ...
    syncSmartSearchPlaceholder();
    bindGuestDrawerToggles();
    initSupportPages();
    initAiChat();
  });
})();
